...
public class SuspendButtonLayout extends RelativeLayout implements View.OnTouchListener {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (suspendedStatus == SUSPEND_BUTTON_CLOSED) {
                    ...
                    if (Math.abs(lastX - downX) > offset && Math.abs(lastY - downY) > offset) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (suspendedStatus == SUSPEND_BUTTON_CLOSED
                        && imageMain.getVisibility() == View.VISIBLE && Math.abs(lastX - downX) < offset && Math.abs(lastY - downY) < offset) {
                ...
                if (suspendedStatus == SUSPEND_BUTTON_OPENED
                    && imageMain.getVisibility() == View.VISIBLE  && Math.abs(lastX - downX) < offset+10 && Math.abs(lastY - downY) < offset) {
    ...
    private void openAnim() {
        ...
        for (int i = 0; i < mNumber; i++) {
            ...
            float endX = startX + x;
            float endY = startY + y;
            moveAnimSingle(imageViewList.get(i), startX, startY, endX, endY, duration, true);
        }
