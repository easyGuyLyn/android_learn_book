...
public class RecordFilterDataUtils {
    ...
    public synchronized static List<NoteRecordHis> recordNoteFilter(
        ...
        for (NoteRecordHis noteRecordHis : hisList) {
            ...
            if (time >= startTime && time <= endTime) {
                if (code.equals("")) {
                    ...
                    } else if (statu == status[4]) {
                        if (noteRecordHis.getStatus().equals(status[4])
                                || noteRecordHis.getStatus().equals(status[5])
                                || noteRecordHis.getStatus().equals(status[6])) {
                            hisListShow.add(noteRecordHis);
                        }

                    } else if (noteRecordHis.getStatus().equals(statu)) {
                        hisListShow.add(noteRecordHis);
                    }
                } else {
     ...
     * @param hisListShow
     */
    private static void sortData(boolean noteSort,
                                 boolean profitSort,
                                 boolean timeSort,
                                 List<NoteRecordHis> hisListShow) {
