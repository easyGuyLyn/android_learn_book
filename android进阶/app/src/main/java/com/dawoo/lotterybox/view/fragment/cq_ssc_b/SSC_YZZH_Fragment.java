...
public class SSC_YZZH_Fragment extends BaseFragment {
    ...
    private void initViews() {
        mList = CQSSCBDataUtils.initYZZHData();
        initTab();
    ...
    private void initData() {
        for (PlayTypeBean.PlayBean playBean : mList) {
            mListLinkedHashMap.put(playBean.getPlayTypeName(), CQSSCBDataUtils.init_YZZH_0_9_Data(
                    playBean.getPlayTypeName(), ""));
        }
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mDetailAdapter.setNewData(mListLinkedHashMap.get(getString(R.string.q5yzzh)));
            }
        });
    }
